...
@Component
public class PostToken implements IEndPointDispatcher {
    private static final Logger logger = LoggerFactory.getLogger(PostToken.class);
    ...
    public String dispatchGet() {
        ...
        try {
            Resource[] resources = resolver.getResources("classpath:templates/endpoint/token/get/*.*");
    ...
    public String dispatchPost() {
        ...
        StringBuilder mockResponse = new StringBuilder("{}");
        try {
            Resource[] resources = resolver.getResources("classpath:templates/endpoint/token/post/*.*");
            if (resources.length > 0) {
                Resource resource = resources[0];
